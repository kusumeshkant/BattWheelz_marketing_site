 ...
 *
 * On fairness of the comparison: a Battwheelz lease includes the vehicle, so
 * the petrol side must include the cost of acquiring one (EMI) and insuring it.
 * Comparing a lease against a bike someone has already paid off would produce a
 ...
 *   battwheelz: { lease: number, charging: number, total: number },
 *   savings: number,
 *   savesMoney: boolean,
 * }}
 */
export function calculateSavings(dailyKm, a) {
  ...
  // Battwheelz side: the lease rate, plus electricity. Servicing, insurance and
  // recovery are inside the lease, which is why they have no line here.
  const lease = a.battwheelzDailyRate * a.workingDaysPerMonth;
  const charging = monthlyKm * a.evChargingCostPerKm;
  const battwheelz = { lease, charging, total: lease + charging };
  ...
  return {
    ...
    battwheelz,
    savings,
    // A lease can genuinely cost more for a very low-mileage rider. The UI says
